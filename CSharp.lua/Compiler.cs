  ...
  public sealed class Compiler {
    ...
    private IEnumerable<string> GetSourceFiles(out bool isDirectory) {
      if (Directory.Exists(input_)) {
        isDirectory = true;
        return RemoveAutogeneratedFolders();
      }
    ...
    private IEnumerable<string> RemoveAutogeneratedFolders() {
      var temp = Directory.EnumerateFiles(input_, "*.cs", SearchOption.AllDirectories);
      string binFolder = Path.Combine(input_, "bin");
      string ObjFolder = Path.Combine(input_, "obj");
      var temp2 = temp.Where(path => !(path.StartsWith(binFolder) || path.StartsWith(ObjFolder)));
      return temp;
    }
